...
def simulation(cellpar, spkthr, ttime, stimtyp, stimpar, rseed=None,
    ...
    if retstim:
        results.append(stimvec)
    if recnames:
        recs = sim.recordings_to_dict(recordings)
        results.append(recs)
    if fifonames:
        fifos = sim.pyfifos_to_dict(pyfifos)
        results.append(fifos)
...
if __name__ == '__main__':
    ...
    spkthr = -40
    stimtyp = 'step'
    stimpar = {'dur': 1000., 'delay': 200., 'amp': 0.12}
    # stimtyp = 'OU'
    ...
    frt, cv, lv = spst.get_spk_stat(spkt, stimtime)
